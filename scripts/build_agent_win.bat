...
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File scripts\build_platform_http_win.ps1 >> agent_build.log 2>&1
if errorlevel 1 goto :fail
set CF=-DCURL_STATICLIB -Itools/curl-win/include -O2 -std=gnu11 -Wall -Wextra -D_GNU_SOURCE -Iinclude -Ivendor/tweetnacl -Ivendor/blake2 -Ivendor/puff -Isrc/platform/win -include src/platform/win/win_compat.h -include %VERSION_HEADER%
...
if errorlevel 1 goto :fail
gcc -c src/common/platform_http.c %CF% -o a_platform_http.o >> agent_build.log 2>&1
if errorlevel 1 goto :fail
gcc -c src/common/platform_proxy.c %CF% -o a_platform_proxy.o >> agent_build.log 2>&1
if errorlevel 1 goto :fail
gcc -c src/tools/platform_agent.c %CF% -o a_platform_agent.o >> agent_build.log 2>&1
if errorlevel 1 goto :fail

echo === link ===>> agent_build.log
gcc -static-libgcc -o idletoken-platform-agent.exe a_platform_agent.o a_platform_http.o a_platform_proxy.o a_sodium_seal.o a_privacy.o a_tweetnacl.o a_blake2b.o a_net.o a_http.o a_b64.o a_apiconv.o a_admission.o a_puff.o a_deflate_wire.o a_win_compat.o a_utf8_manifest.o -Wl,-Bstatic -lwinpthread -Wl,-Bdynamic -lws2_32 -lbcrypt tools/curl-win/lib/libcurl.a -lwinhttp -lcrypt32 -lsecur32 -liphlpapi -lnormaliz -lws2_32 -lbcrypt >> agent_build.log 2>&1
if errorlevel 1 goto :fail
