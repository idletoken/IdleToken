...
/* HomeAI: DSpark's non-causal block attention is CUDA-only for now.
 * Returning 0 makes the drafter unavailable on Metal rather than silently
 * running a causal block, which would look like a working-but-weak drafter.
 * (HomeAI is NVIDIA-only per CLAUDE.md; this exists so an Apple build of
 * upstream ds4 still links.) */
/* HomeAI: Q8_0 single-row dequant — CUDA-only, see the note on the
 * non-causal op below. */
int ds4_gpu_dequant_q8_0_row_tensor(
    ...
    (void)row_len; (void)n_rows; (void)row;
    fprintf(stderr, "ds4: Q8_0 row dequant is not implemented on Metal\n");
    return 0;
}
...
int ds4_gpu_attention_decode_raw_batch_noncausal_heads_tensor(
    ...
    (void)q; (void)raw_kv; (void)n_tokens; (void)pos0; (void)n_raw;
    (void)raw_cap; (void)raw_start; (void)window; (void)n_head; (void)head_dim;
    fprintf(stderr, "ds4: non-causal batch attention is not implemented on Metal\n");
    return 0;
}
