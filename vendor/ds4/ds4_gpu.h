...
/* IdleToken: hybrid VRAM budget in bytes. Must be a call, not an env var —
 * the worker (MinGW) and this DLL (MSVC) do not share a CRT environment. */
void ds4_gpu_set_hybrid_vram_budget(uint64_t bytes);
/* IdleToken: measured ceiling on cudaHostAlloc'd (pinned) host memory, bytes.
 * Returns 0 when it cannot be measured. HYBRID spills every layer that does not
 * fit in VRAM into pinned memory, and this ceiling is far below physical RAM
 * and NOT derivable from it — 73.0% of RAM on one box, 48.2% on another, both
 * exactly reproducible. See tools/pinprobe.cu. Allocating is the only way to
 * find it, so this is expensive and transient; call it once and cache. */
