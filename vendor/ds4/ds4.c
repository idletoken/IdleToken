...
typedef struct {
    uint64_t off;
    uint64_t end;
    /* IdleToken: 0 = "always active" (attention / shared expert / norms /
     * indexer), 1 = routed expert. The accelerator cache is filled greedily in
     * call order, so this is what decides who gets VRAM when the model does not
     * fit: role, not file offset. See accelerator_tensor_span_cmp. */
    int      role;
} accelerator_tensor_span;

/* IdleToken: routed experts LAST.
 *
 * Why this ordering matters more than it looks. GGUF lays tensors out layer by
 * layer, so sorting by offset alone fills VRAM with the first few layers
 * ENTIRE — experts included — and pushes every later layer, attention and all,
 * into host memory. On DSv4 the routed experts are ~91% of each layer
 * (3 x 4096x2048x256) while everything touched on EVERY token — attention,
 * shared expert, norms, indexer — is only ~0.18 GiB per layer, i.e. ~7.6 GiB
 * for all 43 layers. Those 7.6 GiB fit in a 13 GiB card with room for the KV
 * cache; the experts never will. Ordering by role puts the always-active set
 * in VRAM and spills only what is read 6-of-256 times per token.
 * (Measured before this change: GPU kernels at 100% "utilisation" moving
 * 0.98 GB/s — 3.5% of PCIe — because attention weights were being read over
 * the bus on every token. docs/progress.md 2026-08-05.) */
static int accelerator_tensor_span_cmp(const void *a, const void *b) {
    ...
    if (sa->role != sb->role) return sa->role < sb->role ? -1 : 1;
...
/* "blk.N.ffn_{gate,up,down}_exps.weight" — the routed expert tensors. The
 * shared expert (`_shexp`) is deliberately NOT matched: it runs for every
 * token, so it belongs with the always-active set. */
static int tensor_is_routed_expert(const ds4_str *name) {
    if (!name || name->len < 5) return 0;
    const char *p = name->ptr;
    const size_t n = name->len;
    for (size_t i = 0; i + 5 <= n; i++)
        if (memcmp(p + i, "_exps", 5) == 0) return 1;
    return 0;
}
...
static bool accelerator_cache_model_tensor_spans(const ds4_model *m,
    ...
    for (uint64_t i = 0; i < m->n_tensors; i++) {
        ...
        spans[nspan++] = (accelerator_tensor_span){
            ...
            .role = tensor_is_routed_expert(&t->name),
        };
    }
    ...
    uint64_t kept_sum = 0, active_sum = 0, expert_sum = 0;
    for (uint64_t i = 0; i < nspan; i++) {
        const uint64_t b = spans[i].end - spans[i].off;
        kept_sum += b;
        if (spans[i].role) expert_sum += b; else active_sum += b;
    }
    fprintf(stderr,
            "ds4: IdleToken span debug: kept %" PRIu64 " spans totaling %.2f GiB (before merge)\n",
            nspan, (double)kept_sum / 1073741824.0);
    fprintf(stderr,
            "ds4: IdleToken role split: always-active %.2f GiB (cached first), "
            "routed experts %.2f GiB (spill first)\n",
            (double)active_sum / 1073741824.0, (double)expert_sum / 1073741824.0);
    ...
    for (uint64_t i = 0; i < nspan;) {
        ...
        i++;
        const int role = spans[i - 1].role;
        while (i < nspan && spans[i].role == role &&
               spans[i].off <= end + 65536u && spans[i].end - off <= max_span) {
...
typedef struct ds4_layer_weights_s {
...
/* The prediction block has to run in two cases: the probe (measuring accuracy
 * only) or prefetch (actually using it). The first version looked at
 * DS4_PRED_PROBE alone, so setting IDLETOKEN_MOE_PREFETCH=1 did nothing at all
 * -- prefetch was locked inside the probe's switch. */
static int ds4_pred_probe_enabled(void) {
    static int c = -1;
    if (c < 0) {
        const char *p = getenv("DS4_PRED_PROBE");
        const char *f = getenv("IDLETOKEN_MOE_PREFETCH");
        c = ((p && p[0] && p[0] != '0') || (f && f[0] && f[0] != '0')) ? 1 : 0;
    }
    return c;
}
#else
#define ds4_pred_probe_enabled() 0
#define ds4_gpu_pred_stash(a,b) ((void)0)
#define ds4_gpu_pred_check(a,b) ((void)0)
#define ds4_gpu_moe_prefetch(a,b,c,d,e,f,g,h,i) ((void)0)
#endif
...
typedef struct {
    ...
    /* IdleToken cross-layer gate prediction (in the style of Fate): at layer L,
     * run layer L+1's router on the **current** ffn_norm to predict which experts
     * layer L+1 will select. It works because the residual structure makes the
     * gate inputs of adjacent layers highly similar (Fate measured cosine
     * similarity above 83%). The first stage is an accuracy probe only, with no
     * prefetch path wired up. */
    /* Which layer to prefetch for once this layer is done (NULL = no prefetch).
     * It must be issued **after this layer's MoE has been launched**: issued
     * earlier, the prefetch stream and this layer's gather ask PCIe for bandwidth
     * at the same time, and what they contend for is exactly the critical path --
     * measured, stage 0 went from 105.7 to 305.3 ms per token. */
    const struct ds4_layer_weights_s *pf_pending;
    ds4_gpu_tensor *pred_logits;
    ds4_gpu_tensor *pred_probs;
    ds4_gpu_tensor *pred_selected;
    ds4_gpu_tensor *pred_weights;
...
static void metal_graph_free(ds4_gpu_graph *g) {
    ...
    ds4_gpu_tensor_free(g->pred_logits);
    ds4_gpu_tensor_free(g->pred_probs);
    ds4_gpu_tensor_free(g->pred_selected);
    ds4_gpu_tensor_free(g->pred_weights);
...
static bool metal_graph_alloc_raw_cap(
    ...
    g->pred_logits   = ds4_gpu_tensor_alloc(DS4_N_EXPERT * sizeof(float));
    g->pred_probs    = ds4_gpu_tensor_alloc(DS4_N_EXPERT * sizeof(float));
    g->pred_selected = ds4_gpu_tensor_alloc(DS4_N_EXPERT_USED * sizeof(int));
    g->pred_weights  = ds4_gpu_tensor_alloc(DS4_N_EXPERT_USED * sizeof(float));
...
static bool metal_graph_encode_decode_layer(
        ...
        const ds4_model        *model,
        const ds4_layer_weights *layer,
        /* For cross-layer gate prediction: the next layer's weights (NULL if
         * there is none). The layer array does not hang off the model, so the
         * caller has to pass it in. */
        const ds4_layer_weights *next_layer,
        uint32_t                il,
        uint32_t                pos,
        ds4_gpu_tensor       *raw_cache,
        uint32_t                raw_cap,
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
    ...
    /* Cross-layer gate prediction probe (DS4_PRED_PROBE=1).
     *
     * First compare against the prediction the previous layer made for this one
     * (this layer's real selection has just been computed), then run the **next**
     * layer's router on **this** layer's ffn_norm to leave a prediction for it.
     * The order matters: reversed, it would be predicting itself.
     *
     * It reuses the same matmul plus router_select and only swaps which layer's
     * ffn_gate_inp is used -- the predictor is the real router, merely fed a
     * slightly stale input. That is exactly Fate's approach, and nothing needs to
     * be trained. */
    if (ok && ds4_pred_probe_enabled()) {
        ds4_gpu_pred_check(g->router_selected, DS4_N_EXPERT_USED);
        if (next_layer) {
            const ds4_layer_weights *nxt = next_layer;
            if (nxt->ffn_gate_inp &&
                metal_graph_matmul_plain_tensor(g->pred_logits, model, nxt->ffn_gate_inp,
                                                DS4_N_EMBD, DS4_N_EXPERT, g->ffn_norm, 1) &&
                ds4_gpu_router_select_tensor(g->pred_selected, g->pred_weights, g->pred_probs,
                                             model->map, model->size,
                                             nxt->ffn_exp_probs_b ? nxt->ffn_exp_probs_b->abs_offset : 0,
                                             nxt->ffn_gate_tid2eid ? nxt->ffn_gate_tid2eid->abs_offset : 0,
                                             nxt->ffn_gate_tid2eid ? (uint32_t)nxt->ffn_gate_tid2eid->dim[1] : 0,
                                             (uint32_t)token, 0, 0,
                                             nxt->ffn_exp_probs_b != NULL,
                                             nxt->ffn_gate_tid2eid != NULL,
                                             g->pred_logits) != 0) {
                ds4_gpu_pred_stash(g->pred_selected, DS4_N_EXPERT_USED);
                /* The prediction is ready: bring the next layer's experts across
                 * ahead of time on a side stream. This is where the gain is --
                 * the probe only proves the prediction is accurate, whereas
                 * overlapping the transfer is what turns PCIe's 32% of spare
                 * bandwidth into saved time. */
                g->pf_pending = nxt;   /* deferred until after this layer's MoE launch, see pf_pending below */
            }
        }
    }
    ...
    /* This layer's MoE is enqueued, so issue the next layer's prefetch now. The
     * prefetch stream waits on this layer's gather event, so it only uses link
     * idle time **while this layer computes** and never contends with the
     * critical path. */
    if (ok && g->pf_pending) {
        const ds4_layer_weights *nxt = g->pf_pending;
        const uint64_t n_gate_row = routed_expert_row_bytes(nxt->ffn_gate_exps);
        const uint64_t n_down_row = routed_expert_row_bytes(nxt->ffn_down_exps);
        ds4_gpu_moe_prefetch(model->map, model->size,
                             nxt->ffn_gate_exps->abs_offset,
                             nxt->ffn_up_exps->abs_offset,
                             nxt->ffn_down_exps->abs_offset,
                             expert_mid_dim * n_gate_row,
                             routed_out_dim * n_down_row,
                             g->pred_selected, DS4_N_EXPERT_USED);
        g->pf_pending = NULL;
    }
    if (ok) {
...
static int metal_graph_decode_test(
    ...
    if (ok) ok = metal_graph_encode_decode_layer(&g,
                                               model,
                                               layer,
                                               NULL,   /* single-layer debug path: there is no "next layer" */
                                               0,
                                               0,
                                               g.layer_raw_cache[0],
                                               g.raw_cap,
    ...
    if (trace_layers && ok) {
        ...
        for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
            ...
            if (ok) ok = metal_graph_encode_decode_layer(&g, model, &weights->layer[il],
                                                       (il + 1u < DS4_N_LAYER) ? &weights->layer[il + 1u] : NULL,
...
static bool metal_graph_eval_mtp_draft_from_hc(
    ...
    if (ok) {
        ...
        ok = metal_graph_encode_decode_layer(g,
                                             mtp_model,
                                             &mtp->block,
                                             NULL,   /* MTP single block: no next layer */
                                             1,
                                             pos,
                                             g->mtp_raw_cache,
                                             g->raw_cap,
