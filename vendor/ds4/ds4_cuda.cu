...
#include <cublas_v2.h>
#ifdef _WIN32
#include <windows.h>   /* LoadLibraryA — cuBLAS is delay-loaded, see cuda_init */
#endif
...
static uint64_t g_hybrid_vram_bytes;  /* HYBRID: weights kept in device VRAM */
static uint64_t g_hybrid_ram_bytes;   /* HYBRID: weights spilled to pinned host RAM */
...
static const char *cuda_model_range_ptr(const void *model_map, uint64_t offset, uint64_t bytes, const char *what) {
...
#ifdef _WIN32
    /* HYBRID (IDLETOKEN_HYBRID=1): keep up to a VRAM budget of weights resident in
     * device memory (cudaMalloc, fast), and spill everything beyond it into
     * PINNED, MAPPED host memory (cudaHostAlloc) that the GPU reads over PCIe.
     * This uses BOTH the card's VRAM and its RAM. (Managed-memory
     * oversubscription is NOT usable here: WDDM caps it at physical VRAM and
    ...
    if (g_hybrid_budget_bytes || getenv("IDLETOKEN_HYBRID")) {
        ...
        if (!to_ram) {
            void *dvr = NULL;
            if (cudaMalloc(&dvr, (size_t)bytes) == cudaSuccess) {
                char *stg = (char *)malloc((size_t)(bytes < chunk ? bytes : chunk));
                int ok = (stg != NULL);
                for (uint64_t d = 0; ok && d < bytes; d += chunk) {
                ...
                free(stg);
                if (ok) {
                    g_hybrid_vram_bytes += bytes;
                    g_model_ranges.push_back({model_map, offset, bytes, (char *)dvr, NULL, NULL, 0, 0, 0});
                    g_model_range_by_offset[offset] = g_model_ranges.size() - 1u;
                    g_model_range_bytes += bytes;
                    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE"))
                        fprintf(stderr, "ds4: CUDA hybrid VRAM %s %.0f MiB (vram %.2f/%.2f GiB)\n",
                                what?what:"w", (double)bytes/1048576.0,
                                (double)g_hybrid_vram_bytes/1073741824.0, (double)vram_budget/1073741824.0);
                    return (const char *)dvr;
                }
                (void)cudaFree(dvr); (void)cudaGetLastError();
            } else { (void)cudaGetLastError(); }
            /* VRAM alloc/copy failed → fall through to host-pinned spill. */
        }
        void *hptr = NULL;
        err = cudaHostAlloc(&hptr, (size_t)bytes, cudaHostAllocMapped);
        if (err != cudaSuccess) {
            (void)cudaGetLastError();
            fprintf(stderr, "ds4: CUDA hybrid host-alloc failed for %s (%.2f MiB): %s\n",
        ...
        if (cudaHostGetDevicePointer(&dptr, hptr, 0) != cudaSuccess || !dptr) {
            (void)cudaGetLastError();
            fprintf(stderr, "ds4: CUDA hybrid host device-ptr failed for %s\n", what?what:"weights");
            (void)cudaFreeHost(hptr);
            return NULL;
        }
        g_hybrid_ram_bytes += bytes;
        g_model_ranges.push_back({model_map, offset, bytes, (char *)dptr, hptr, NULL, bytes, 0, 0});
        g_model_range_by_offset[offset] = g_model_ranges.size() - 1u;
        g_model_range_bytes += bytes;
        if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE"))
            fprintf(stderr, "ds4: CUDA hybrid RAM  %s %.0f MiB (ram %.2f GiB over PCIe)\n",
                    what?what:"w", (double)bytes/1048576.0, (double)g_hybrid_ram_bytes/1073741824.0);
...
static void cuda_model_range_release_all(void) {
    ...
    g_hybrid_vram_bytes = 0;
    g_hybrid_ram_bytes = 0;
...
extern "C" int ds4_gpu_init(void) {
    ...
    /* IdleToken patch: DS4_NO_CUBLAS=1 leaves the handle unset, so every
     * `if (g_cublas_ready && n_tok > 1)` fast path below falls through to the
     * native kernels. Purpose is measurement, not a feature: cuBLAS costs
     * 768 MB of DLLs in the Windows installer (out of 447 MiB compressed) and
     * is used in exactly five prefill GEMMs, all of which already have a
     * fallback. Before paying to remove the dependency we need the number it
     * actually buys — see docs/review/findings.md R-09. */
    if (!g_cublas_ready && getenv("DS4_NO_CUBLAS")) {
        fprintf(stderr, "ds4: DS4_NO_CUBLAS=1 — prefill GEMMs use the native "
                        "kernels (slower; measurement mode)\n");
    }
#ifdef _WIN32
    /* We no longer SHIP cuBLAS: those two DLLs were 769 MB of a 790 MB payload
     * (97%), and they come from the CUDA Toolkit, not the display driver
     * (product decision 2026-08-04, docs/acceptance-criteria.md philosophy 12).
     *
     * ds4cuda.dll is linked with /DELAYLOAD:cublas64_12.dll so its ABSENCE is
     * not fatal — without that, Windows kills the process with 0xC0000135
     * BEFORE main(), and "tell the user to install it" is impossible from a
     * process that never runs. Probe it here, once, before the first cuBLAS
     * call; every call site below is already guarded by g_cublas_ready, so
     * leaving it 0 simply routes prefill through the native kernels. */
    else if (!g_cublas_ready) {
        HMODULE cb = LoadLibraryA("cublas64_12.dll");
        if (!cb) {
            fprintf(stderr,
                "ds4: cublas64_12.dll not found - prefill will use the built-in\n"
                "     kernels: correct, but measured 9.1x slower on a 900-token\n"
                "     prompt (2.7s -> 24.6s).\n"
                "     For full speed install the NVIDIA CUDA Toolkit 12.x:\n"
                "       https://developer.nvidia.com/cuda-downloads\n"
                "     The display driver alone does NOT include cuBLAS.\n");
        } else {
            FreeLibrary(cb);   /* the delay-load stub will load it for real */
...
/* IdleToken: a trace of expert selection (diagnostic, DS4_CUDA_MOE_TRACE=1).
 *
 * Before writing a "hot expert VRAM cache" one question can veto the whole plan:
 * **is expert selection skewed enough to exploit at all?** top-6 out of 256; if
 * it is close to uniform over time, then no matter how much is cached the hit
 * rate is merely proportional and buys no order of magnitude -- and that
 * engineering should not be spent. The same trace answers DSpark's precondition
 * too: how much the expert sets of adjacent tokens overlap. Two questions, one
 * measurement.
 *
 * Recording is a **device-to-device copy** on the same stream: `selected` must
 * not be read back to the host -- synchronizing 43 times per token is exactly
 * what once brought a machine down. One D2H happens only when the buffer fills,
 * which amortized over 64 tokens is negligible. */
static int32_t  *g_sel_trace = NULL;
static uint32_t  g_sel_trace_n = 0;
static uint32_t  g_sel_trace_cap = 0;
static uint32_t  g_sel_trace_stride = 0;
static int       g_sel_trace_dumped = 0;

static void cuda_sel_trace_record(const int32_t *selected, uint32_t n_expert) {
    if (g_sel_trace_dumped) return;
    if (!g_sel_trace) {
        /* The capacity must be **small enough to actually fill**: the dump only
         * happens when it does. The first version sized it as 64 tokens x 43
         * layers x 8 slots = 22016 ints, while a benchmark round writes
         * 31 x 28 x 6 = 5208 -- it could never fill, so not one line of trace was
         * ever printed. Size it against what is really written: 4096 by default,
         * raised with DS4_CUDA_MOE_TRACE=<number of ints>. */
        g_sel_trace_cap = 4096u;
        {
            const char *e = getenv("DS4_CUDA_MOE_TRACE");
            if (e && e[0]) {
                unsigned long v = strtoul(e, NULL, 10);
                if (v >= 256ul && v <= (1ul << 22)) g_sel_trace_cap = (uint32_t)v;
            }
        }
        if (cudaMalloc(&g_sel_trace, (size_t)g_sel_trace_cap * sizeof(int32_t)) != cudaSuccess) {
            (void)cudaGetLastError(); g_sel_trace = NULL; g_sel_trace_dumped = 1; return;
        }
        g_sel_trace_stride = n_expert;
    }
    if (g_sel_trace_n + n_expert > g_sel_trace_cap) {
        int32_t *host = (int32_t *)malloc((size_t)g_sel_trace_n * sizeof(int32_t));
        if (host && cudaMemcpy(host, g_sel_trace, (size_t)g_sel_trace_n * sizeof(int32_t),
                               cudaMemcpyDeviceToHost) == cudaSuccess) {
            fprintf(stderr, "ds4: MoE TRACE begin stride=%u n=%u\n", g_sel_trace_stride, g_sel_trace_n);
            for (uint32_t i = 0; i + g_sel_trace_stride <= g_sel_trace_n; i += g_sel_trace_stride) {
                fprintf(stderr, "ds4: TRACE %u", i / g_sel_trace_stride);
                for (uint32_t k = 0; k < g_sel_trace_stride; k++) fprintf(stderr, " %d", host[i + k]);
                fprintf(stderr, "\n");
            }
            fprintf(stderr, "ds4: MoE TRACE end\n");
        }
        free(host);
        /* **Keep recording** after a dump rather than stopping. Deciding whether
         * a static hot-expert set exists needs roughly 30 observations per expert
         * per layer, i.e. about 1280 tokens per layer, and one buffer of 4096 ints
         * is nowhere near enough; a tool that dumps once can only conclude "cannot
         * tell", which is exactly why the previous round's conclusion was
         * overstated. */
        g_sel_trace_n = 0;
        return;
    }
...
/* The worker passes the cache budget explicitly as **total bytes**, not as slots
 * per layer.
 *
 * Why it must be a total: `vram_usable` is reported in RESOURCE_REPORT, which
 * happens **before** ASSIGN_PLAN arrives and the node learns how many layers it
 * has. A total is independent of the layer count and can be subtracted on the
 * spot; slots per layer cannot. The first version used slots per layer, and the
 * planner had no idea how much VRAM the cache consumed -- the split on identical
 * hardware drifted from 28/15 to 16/27, making two measurements incomparable.
 *
 * The slot count follows once the layer count is known:
 * slots = total bytes / (layers x bytes per expert).
 * The `IDLETOKEN_MOE_CACHE=<slots>` environment variable still takes precedence --
 * it is the only way to A/B without recompiling. */
static uint64_t g_moe_cache_budget_bytes = 0;
static uint32_t g_moe_cache_layers = 0;

/* =====================================================================
 * IdleToken: cross-layer prediction plus double-buffered prefetch
 *
 * The basis (measured on real hardware):
 *   - cross-layer gate prediction recalls **73.7%** (random is 2.3%), see
 *     DS4_PRED_PROBE;
 *   - the PCIe link has **32% headroom**: the same scattered-read pattern
 *     measures 17.11 GB/s in isolation but only 11.6 GB/s inside the engine.
 *     Claiming it requires **overlapping transfers across layers**, since within
 *     a layer the gather and the compute are dependent and cannot overlap.
 *
 * Why double buffering rather than a per-layer cache: layers execute strictly in
 * order, so at any moment only the "next layer's" copy is needed. A per-layer LRU
 * cache would want 28 layers x 12 slots = 2.3 GiB, while double buffering needs
 * 2 x 12 slots = 162 MiB -- which sidesteps exactly the zero-sum VRAM problem
 * that stalls the LRU cache (every GiB the cache takes must be given up by the
 * resident weights, and resident weights hit 100%).
 *
 * Slot layout: the first n_sel hold what prediction brought in, the last n_sel
 * are reserved for filling in on demand when the prediction was wrong.
 * ===================================================================== */
/* Shares the miss-fill kernel with the LRU cache (defined in the cache section
 * below). */
...
#define IDLETOKEN_PF_SLOTS (2u * IDLETOKEN_MOE_CACHE_MAX_SEL)

struct moe_prefetch_buf {
    char     *gate, *up, *down;
    int32_t  *ids;        /* [PF_SLOTS] which expert each slot holds, -1 = empty */
    int32_t  *out_slots;  /* [n_sel] the slot each actually-selected expert is in */
    int32_t  *miss_src;   /* [n_sel] experts mispredicted and needing a fill */
    int32_t  *miss_dst;   /* [n_sel] their slots */
    int32_t  *n_miss;
    uint64_t  key;        /* which layer this prefetch is for (gate_offset), 0 = invalid */
    uint64_t  gate_bytes, down_bytes;
    uint32_t  n_pre;
    int       inited;
};
static moe_prefetch_buf g_pf[2];
static uint32_t     g_pf_cur = 0;          /* the buffer the next layer will consume */
static cudaStream_t g_pf_stream = NULL;
/* Marks this layer's gather as complete. The prefetch stream **must wait on
 * it** -- otherwise the prefetch and the current layer's gather ask PCIe for
 * bandwidth simultaneously, contending for exactly the critical path, which makes
 * it a queue-jump rather than a head start. The first version had no such event,
 * and stage 0 measured 105.7 to 305.3 ms per token. */
static cudaEvent_t  g_gather_done = NULL;
static cudaEvent_t  g_pf_evt[2] = {NULL, NULL};
static unsigned long long *g_pf_stat = NULL;   /* [0]=prefetch hits [1]=total demand */
static unsigned long long  g_pf_calls = 0;

static int moe_prefetch_on(void) {
    static int c = -1;
    if (c < 0) { const char *e = getenv("IDLETOKEN_MOE_PREFETCH"); c = (e && e[0] && e[0] != '0') ? 1 : 0; }
    return c;
}

/* Record the predicted expert indices in the slot table (no data movement; the
 * fill kernel does that). */
__global__ static void pf_plan_kernel(int32_t *ids, const int32_t *pred, uint32_t n_sel,
                                      uint32_t n_total, int32_t *miss_src, int32_t *miss_dst,
                                      int32_t *n_miss) {
    if (blockIdx.x || threadIdx.x) return;
    for (uint32_t i = 0; i < IDLETOKEN_PF_SLOTS; i++) ids[i] = -1;
    for (uint32_t i = 0; i < n_sel; i++) {
        int32_t e = pred[i];
        if (e < 0 || (uint32_t)e >= n_total) e = 0;
        ids[i] = e; miss_src[i] = e; miss_dst[i] = (int32_t)i;
    }
    *n_miss = (int32_t)n_sel;
}

/* Consume: an actually-selected expert already in a prefetch slot is a hit;
 * otherwise assign an empty slot from the second half and record it as pending. */
__global__ static void pf_consume_kernel(int32_t *ids, const int32_t *actual, uint32_t n_sel,
                                         uint32_t n_total, int32_t *out_slots,
                                         int32_t *miss_src, int32_t *miss_dst, int32_t *n_miss,
                                         unsigned long long *stat) {
    if (blockIdx.x || threadIdx.x) return;
    int nm = 0;
    uint32_t spare = n_sel;   /* start of the second half */
    for (uint32_t i = 0; i < n_sel; i++) {
        int32_t e = actual[i];
        if (e < 0 || (uint32_t)e >= n_total) e = 0;
        int32_t slot = -1;
        for (uint32_t j = 0; j < n_sel; j++) if (ids[j] == e) { slot = (int32_t)j; break; }
        if (stat) atomicAdd(&stat[1], 1ull);
        if (slot >= 0) { if (stat) atomicAdd(&stat[0], 1ull); out_slots[i] = slot; continue; }
        slot = (int32_t)spare++;
        if ((uint32_t)slot >= IDLETOKEN_PF_SLOTS) slot = (int32_t)(IDLETOKEN_PF_SLOTS - 1u);
        ids[slot] = e;
        miss_src[nm] = e; miss_dst[nm] = slot; nm++;
        out_slots[i] = slot;
    }
    *n_miss = nm;
}

static int moe_prefetch_alloc(moe_prefetch_buf *b, uint32_t n_sel,
                              uint64_t gate_bytes, uint64_t down_bytes) {
    if (b->inited) {
        if (b->gate_bytes != gate_bytes || b->down_bytes != down_bytes) return 0;
        return 1;
    }
    const size_t gb = (size_t)gate_bytes * IDLETOKEN_PF_SLOTS;
    const size_t db = (size_t)down_bytes * IDLETOKEN_PF_SLOTS;
    int ok = cudaMalloc((void **)&b->gate, gb) == cudaSuccess
          && cudaMalloc((void **)&b->up,   gb) == cudaSuccess
          && cudaMalloc((void **)&b->down, db) == cudaSuccess
          && cudaMalloc((void **)&b->ids,       IDLETOKEN_PF_SLOTS * sizeof(int32_t)) == cudaSuccess
          && cudaMalloc((void **)&b->out_slots, (size_t)n_sel * sizeof(int32_t)) == cudaSuccess
          && cudaMalloc((void **)&b->miss_src,  IDLETOKEN_PF_SLOTS * sizeof(int32_t)) == cudaSuccess
          && cudaMalloc((void **)&b->miss_dst,  IDLETOKEN_PF_SLOTS * sizeof(int32_t)) == cudaSuccess
          && cudaMalloc((void **)&b->n_miss,    sizeof(int32_t)) == cudaSuccess;
    if (!ok) { (void)cudaGetLastError(); return 0; }
    b->gate_bytes = gate_bytes; b->down_bytes = down_bytes; b->inited = 1; b->key = 0;
    return 1;
}

/* Called at layer L: bring the **predicted** experts of layer L+1 across ahead of
 * time on a side stream. The side stream is essential -- on the default stream it
 * would still be serial and nothing would overlap. */
extern "C" void ds4_gpu_moe_prefetch(const void *model_map, uint64_t model_size,
    ...
    if (!moe_prefetch_on() || !predicted || !predicted->ptr) return;
    if (n_sel == 0 || n_sel > IDLETOKEN_MOE_CACHE_MAX_SEL) return;
    /* Prefetching is only worthwhile when the weights really are in host memory;
     * moving something already in VRAM achieves nothing. */
    if (!cuda_model_range_is_host(gate_offset, gate_expert_bytes * 256ull)) return;
    if (!g_pf_stream) {
        if (cudaStreamCreateWithFlags(&g_pf_stream, cudaStreamNonBlocking) != cudaSuccess) return;
        (void)cudaEventCreateWithFlags(&g_pf_evt[0], cudaEventDisableTiming);
        (void)cudaEventCreateWithFlags(&g_pf_evt[1], cudaEventDisableTiming);
        (void)cudaMalloc((void **)&g_pf_stat, 2 * sizeof(unsigned long long));
        if (g_pf_stat) (void)cudaMemset(g_pf_stat, 0, 2 * sizeof(unsigned long long));
    }
    const uint32_t idx = 1u - g_pf_cur;           /* write the other buffer; this layer consumes the current one */
    moe_prefetch_buf *b = &g_pf[idx];
    if (!moe_prefetch_alloc(b, n_sel, gate_expert_bytes, down_expert_bytes)) return;

    const char *gw = cuda_model_range_ptr(model_map, gate_offset, gate_expert_bytes * 256ull, "pf_gate");
    const char *uw = cuda_model_range_ptr(model_map, up_offset,   gate_expert_bytes * 256ull, "pf_up");
    const char *dw = cuda_model_range_ptr(model_map, down_offset, down_expert_bytes * 256ull, "pf_down");
    if (!gw || !uw || !dw) return;

    /* The crux: start only after this layer's gather has finished, so it uses only
     * the link's idle time during compute. */
    if (g_gather_done) (void)cudaStreamWaitEvent(g_pf_stream, g_gather_done, 0);
    pf_plan_kernel<<<1, 1, 0, g_pf_stream>>>(b->ids, (const int32_t *)predicted->ptr,
                                             n_sel, 256u, b->miss_src, b->miss_dst, b->n_miss);
    const uint32_t th = 256u;
    uint32_t gb = (uint32_t)((gate_expert_bytes / 16u + th - 1) / th); if (gb > 1024u) gb = 1024u; if (!gb) gb = 1u;
    uint32_t db = (uint32_t)((down_expert_bytes / 16u + th - 1) / th); if (db > 1024u) db = 1024u; if (!db) db = 1u;
    dim3 gg(gb, n_sel, 1), dg(db, n_sel, 1);
    moe_cache_fill_kernel<<<gg, th, 0, g_pf_stream>>>(b->gate, gw, b->miss_src, b->miss_dst, b->n_miss, gate_expert_bytes, 256u);
    moe_cache_fill_kernel<<<gg, th, 0, g_pf_stream>>>(b->up,   uw, b->miss_src, b->miss_dst, b->n_miss, gate_expert_bytes, 256u);
    moe_cache_fill_kernel<<<dg, th, 0, g_pf_stream>>>(b->down, dw, b->miss_src, b->miss_dst, b->n_miss, down_expert_bytes, 256u);
    (void)cudaEventRecord(g_pf_evt[idx], g_pf_stream);
    b->key = gate_offset; b->n_pre = n_sel;
    (void)cudaGetLastError();
}

/* Called at layer L+1 (from the decode branch of routed_moe_launch): consume the
 * prefetch. Returning 1 means it took over and the caller's gate_w and sel_ptr
 * have been rewritten. */
static int cuda_moe_prefetch_decode(const char **gate_w, const char **up_w, const char **down_w,
                                    const int32_t **selected_out, const int32_t *selected,
                                    uint32_t n_expert, uint64_t gate_expert_bytes,
                                    uint64_t down_expert_bytes, uint64_t layer_key) {
    if (!moe_prefetch_on() || !g_pf_stream) return 0;
    moe_prefetch_buf *b = &g_pf[g_pf_cur];
    /* A key mismatch means the previous layer did not prefetch for this one (the
     * first layer, or a failed prefetch), so take the original path. */
    if (!b->inited || b->key != layer_key || b->gate_bytes != gate_expert_bytes) return 0;

    (void)cudaStreamWaitEvent(0, g_pf_evt[g_pf_cur], 0);   /* wait for this prefetch to land */
    pf_consume_kernel<<<1, 1>>>(b->ids, selected, n_expert, 256u, b->out_slots,
                                b->miss_src, b->miss_dst, b->n_miss, g_pf_stat);
    const uint32_t th = 256u;
    uint32_t gb = (uint32_t)((gate_expert_bytes / 16u + th - 1) / th); if (gb > 1024u) gb = 1024u; if (!gb) gb = 1u;
    uint32_t db = (uint32_t)((down_expert_bytes / 16u + th - 1) / th); if (db > 1024u) db = 1024u; if (!db) db = 1u;
    dim3 gg(gb, n_expert, 1), dg(db, n_expert, 1);
    /* Fill in only the mispredicted ones. */
    moe_cache_fill_kernel<<<gg, th>>>(b->gate, *gate_w, b->miss_src, b->miss_dst, b->n_miss, gate_expert_bytes, 256u);
    moe_cache_fill_kernel<<<gg, th>>>(b->up,   *up_w,   b->miss_src, b->miss_dst, b->n_miss, gate_expert_bytes, 256u);
    moe_cache_fill_kernel<<<dg, th>>>(b->down, *down_w, b->miss_src, b->miss_dst, b->n_miss, down_expert_bytes, 256u);
    if (cudaGetLastError() != cudaSuccess) return 0;

    *gate_w = b->gate; *up_w = b->up; *down_w = b->down;
    *selected_out = b->out_slots;
    b->key = 0;
    g_pf_cur = 1u - g_pf_cur;      /* swap buffers: the next layer consumes what this one just wrote */

    if (++g_pf_calls % 512ull == 0ull && g_pf_stat) {
        unsigned long long h[2] = {0, 0};
        if (cudaMemcpy(h, g_pf_stat, sizeof(h), cudaMemcpyDeviceToHost) == cudaSuccess && h[1])
            fprintf(stderr, "ds4: MoE prefetch hit %llu/%llu = %.1f%%\n",
                    h[0], h[1], 100.0 * (double)h[0] / (double)h[1]);
    }
    return 1;
}

/* IdleToken: an accuracy probe for cross-layer gate prediction (DS4_PRED_PROBE=1).
 *
 * The mechanism (Fate, arXiv:2502.12224): the residual structure makes the
 * **inputs** of adjacent layers' gates highly similar (the paper measures cosine
 * similarity above 83%), so running layer L+1's router on the current hidden
 * state at layer L yields an approximate selection of L+1's experts in advance.
 * The paper reports 78.79% during decode.
 *
 * Why only a probe first: three mechanisms from papers have already failed on
 * DSv4 (ARC, non-uniform per-layer allocation, ST-MoE's CCT), all for the same
 * reason -- **fine-grained routing, 6 of 256**. The models in those papers mostly
 * pick 2-6 of 8-64, where the random baseline is an order of magnitude higher. So
 * spend 30 lines measuring the accuracy properly before deciding whether to build
 * the prefetch path.
 *
 * Zero host synchronization throughout: comparison and counting happen on the
 * device, read back only every N calls. */
static int32_t           *g_pred_stash = NULL;      /* the top-k the previous layer predicted for this one */
static int32_t           *g_pred_valid = NULL;      /* [1] whether the stash belongs to this layer */
static unsigned long long *g_pred_stat = NULL;      /* [0]=hits [1]=total */
static unsigned long long  g_pred_calls = 0;

__global__ static void pred_stash_kernel(int32_t *stash, int32_t *valid,
                                         const int32_t *pred, uint32_t n) {
    if (blockIdx.x || threadIdx.x) return;
    for (uint32_t i = 0; i < n; i++) stash[i] = pred[i];
    *valid = 1;
}

__global__ static void pred_check_kernel(const int32_t *stash, int32_t *valid,
                                         const int32_t *actual, uint32_t n,
                                         unsigned long long *stat) {
    if (blockIdx.x || threadIdx.x) return;
    if (!*valid) return;
    for (uint32_t i = 0; i < n; i++) {
        int32_t a = actual[i];
        int hit = 0;
        for (uint32_t j = 0; j < n; j++) if (stash[j] == a) { hit = 1; break; }
        atomicAdd(&stat[1], 1ull);
        if (hit) atomicAdd(&stat[0], 1ull);
    }
    *valid = 0;
}

static int pred_probe_on(void) {
    static int c = -1;
    if (c < 0) { const char *e = getenv("DS4_PRED_PROBE"); c = (e && e[0] && e[0] != '0') ? 1 : 0; }
    return c;
}

static int pred_probe_alloc(void) {
    if (g_pred_stash) return 1;
    if (cudaMalloc((void **)&g_pred_stash, 16 * sizeof(int32_t)) != cudaSuccess) return 0;
    if (cudaMalloc((void **)&g_pred_valid, sizeof(int32_t)) != cudaSuccess) return 0;
    if (cudaMalloc((void **)&g_pred_stat, 2 * sizeof(unsigned long long)) != cudaSuccess) return 0;
    (void)cudaMemset(g_pred_valid, 0, sizeof(int32_t));
    (void)cudaMemset(g_pred_stat, 0, 2 * sizeof(unsigned long long));
    return 1;
}

/* Called at layer L: stash the top-k predicted for layer L+1. */
extern "C" void ds4_gpu_pred_stash(const ds4_gpu_tensor *pred_sel, uint32_t n) {
    if (!pred_probe_on() || !pred_sel || !pred_sel->ptr || n == 0 || n > 16u) return;
    if (!pred_probe_alloc()) return;
    pred_stash_kernel<<<1, 1>>>(g_pred_stash, g_pred_valid, (const int32_t *)pred_sel->ptr, n);
}

/* Called at layer L+1: compare the real selection against the previous layer's
 * stashed prediction. */
extern "C" void ds4_gpu_pred_check(const ds4_gpu_tensor *actual_sel, uint32_t n) {
    if (!pred_probe_on() || !actual_sel || !actual_sel->ptr || n == 0 || n > 16u || !g_pred_stash) return;
    pred_check_kernel<<<1, 1>>>(g_pred_stash, g_pred_valid, (const int32_t *)actual_sel->ptr, n, g_pred_stat);
    if (++g_pred_calls % 512ull == 0ull) {
        unsigned long long h[2] = {0, 0};
        if (cudaMemcpy(h, g_pred_stat, sizeof(h), cudaMemcpyDeviceToHost) == cudaSuccess && h[1])
            fprintf(stderr, "ds4: cross-layer gate prediction recall %llu/%llu = %.1f%% (random %.1f%%)\n",
                    h[0], h[1], 100.0 * (double)h[0] / (double)h[1], 100.0 * (double)n / 256.0);
    }
}

extern "C" void ds4_gpu_set_moe_cache(uint64_t bytes, uint32_t n_layers) {
    g_moe_cache_budget_bytes = bytes;
    g_moe_cache_layers = n_layers;
    fprintf(stderr, "ds4: IdleToken MoE cache budget %.2f GiB over %u layers\n",
            (double)bytes / 1073741824.0, n_layers);
}

/* Capacity is derived from **the VRAM this machine actually has free right
 * now**; two machines must not share one number.
 *
 * The measured cliff: on a 28-layer host, 12 slots = 2.21 GiB gave gpuexec
 * 47.8 ms; raising it to 16 slots = 2.95 GiB moved the hit rate from 49.1% to
 * 55.5% as expected, **but gpuexec exploded to 243.6 ms, a factor of 5** -- VRAM
 * was drained and the CUDA allocator entered WDDM oversubscription and paging,
 * which is the very first problem this project ever fixed. In the same round a
 * 15-layer host with fewer resident weights was untouched at 42.7 ms.
 *
 * So this is not "diminishing returns" but "a little more and it collapses", and
 * the collapse point **differs per machine** -- the assigned layer count, the
 * resident weights, and what is left on the card all differ. The same class of
 * problem as the pinned-memory ceiling.
 *
 * MARGIN keeps 1.0 GiB: after the cache is allocated, the prefill batch buffers
 * and temporary tensors still need room to live. */
static uint32_t moe_cache_slots_cfg(uint64_t per_expert_bytes) {
    if (g_moe_cache_slots) return g_moe_cache_slots;   /* computed once */

    const char *e = getenv("IDLETOKEN_MOE_CACHE");
    if (e && e[0]) {                                    /* explicit override wins */
        unsigned long v = strtoul(e, NULL, 10);
        if (v == 0ul) return 0;                         /* =0 disables it explicitly */
        if (v >= IDLETOKEN_MOE_CACHE_MAX_SEL && v <= 256ul) {
            g_moe_cache_slots = (uint32_t)v;
            fprintf(stderr, "ds4: MoE cache %u slots/layer (manual)\n", g_moe_cache_slots);
            return g_moe_cache_slots;
        }
    }
    if (!g_moe_cache_layers || !per_expert_bytes) return 0;

    uint64_t budget = g_moe_cache_budget_bytes;         /* explicit total, optional */
    if (!budget) {
        /* Automatic sizing **must not use cudaMemGetInfo's free**. WDDM allows
         * allocating beyond what it reports (falling back to system memory), so
         * that value badly underestimates what is available -- measured, a manual
         * 12 slots allocated 2.21 GiB successfully and ran fast (6.634s), while
         * cudaMemGetInfo reported free=1.49 GiB at that same moment. Sizing
         * automatically from it would compute 2 slots and disable the cache
         * outright, on exactly the machine that needs it most.
         *
         * Use the accounting we control instead: total VRAM minus the HYBRID
         * weight budget minus a fixed reserve. The 2.7 GiB reserve covers the CUDA
         * context, the KV cache and the prefill batch buffers; on a host with
         * 15.93 GiB total, an 11.0 GiB budget and 28 layers it yields about 12
         * slots, right in the measured safe zone (12 stable, 16 collapses). */
        size_t vfree = 0, vtotal = 0;
        if (cudaMemGetInfo(&vfree, &vtotal) != cudaSuccess) { (void)cudaGetLastError(); return 0; }
        const uint64_t RESERVE = 2899102924ull;         /* 2.7 GiB */
        const uint64_t weights = g_hybrid_budget_bytes ? g_hybrid_budget_bytes : (11ull << 30);
        uint64_t used = weights + RESERVE;
        budget = (uint64_t)vtotal > used ? (uint64_t)vtotal - used : 0;
        fprintf(stderr, "ds4: MoE cache auto -- vram %.2f GiB - weights %.2f - reserve 2.70 "
                        "-> budget %.2f GiB (cudaMemGetInfo free=%.2f, for reference only)\n",
                (double)vtotal / 1073741824.0, (double)weights / 1073741824.0,
                (double)budget / 1073741824.0, (double)vfree / 1073741824.0);
    }
    uint64_t s = budget / ((uint64_t)g_moe_cache_layers * per_expert_bytes);
    if (s < IDLETOKEN_MOE_CACHE_MAX_SEL) {              /* cannot hold one top-k: do not enable */
        fprintf(stderr, "ds4: MoE cache disabled -- headroom only allows %llu slots/layer\n", (unsigned long long)s);
        return 0;
    }
    if (s > 64ull) s = 64ull;                           /* more buys no further hits, see the Belady bound */
    g_moe_cache_slots = (uint32_t)s;
    fprintf(stderr, "ds4: MoE cache %u slots/layer (auto, %u layers)\n",
            g_moe_cache_slots, g_moe_cache_layers);
    return g_moe_cache_slots;
}
...
static int cuda_moe_cache_decode(const char **gate_w, const char **up_w, const char **down_w,
                                 ...
                                 uint64_t layer_key) {
    const uint32_t slots = moe_cache_slots_cfg(2ull * gate_expert_bytes + down_expert_bytes);
...
static int cuda_moe_stage_decode(const char **gate_w, const char **up_w, const char **down_w,
    ...
    if (cudaGetLastError() != cudaSuccess) return 0;
    if (moe_prefetch_on()) {
        if (!g_gather_done) (void)cudaEventCreateWithFlags(&g_gather_done, cudaEventDisableTiming);
        if (g_gather_done) (void)cudaEventRecord(g_gather_done, 0);
    }
...
static int routed_moe_launch(
    ...
    /* Trace recording must live **outside** the staging branch: routing decisions
     * are model behaviour and have nothing to do with whether the weights sit in
     * VRAM or host memory. Inside the branch, a unified-memory machine (where
     * `cuda_model_range_is_host` is false) would record nothing at all -- and that
     * is the only machine that can push a full dataset through quickly. */
    if (n_tokens == 1u && getenv("DS4_CUDA_MOE_TRACE"))
        cuda_sel_trace_record((const int32_t *)selected->ptr, n_expert);
    ...
    if (cuda_moe_stage_enabled() && cuda_model_range_is_host(gate_offset, gate_bytes)) {
        ...
        const int staged = (n_tokens == 1u)
            ? (cuda_moe_prefetch_decode(&gate_w, &up_w, &down_w, &sel_ptr,
                                        (const int32_t *)selected->ptr, n_expert,
                                        gate_expert_bytes, down_expert_bytes, gate_offset)
