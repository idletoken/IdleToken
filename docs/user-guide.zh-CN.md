...
- 上下文长度有 128K、256K 和 1M 三档。新安装默认选择 128K；只有模型支持的档位
  才会显示。
- “仅用本机”和“多台机器一起”两个入口都会显示。本机资源足够时，客户端突出显示
  本机运行；否则突出显示多机运行。只要设备满足对应的硬件要求，就可以手动选择另一种方式。
...
glibc 2.39 或更新版本；不满足时，包管理器会拒绝安装。官方 Linux 安装包只有 `.deb`
和 `.rpm`，其他发行版需要从公开仓库构建。
...
1. 打开 **集群** 页面。
2. 在“已选模型”一栏选择模型与精度。
3. 选择 128K、256K 或 1M 上下文。客户端只显示该模型支持的档位；窗口越大，通常需要
   越多显存或统一内存。
4. 查看资源卡。
...
- 选择 **仅在本机运行**：不经过局域网 RPC，配置最简单。
- 选择 **创建集群**：让多台机器共同承载模型；也可以在随后打开的窗口里加入已有集群。

按钮的主次样式只是根据当前估算提供的提示，不会替你做最终选择。启动时如果资源不足，
客户端会显示 `[RESOURCE_INSUFFICIENT]` 及需要量、可用量；可以降低精度、选择更短的上下文、
释放显存，或增加计算节点后重试。
...
- 根据资源卡和错误详情释放显存/内存，或选择更低精度、更短上下文。
