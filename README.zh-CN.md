...
智能体任务天生具有突发性。多数时候可能只有一两个推理任务在运行；复杂任务到来时，多个智能体会同时工作，一批并行请求会在短时间内把算力需求推高。

家里的机器也有自己的使用节奏：有时繁忙，大多数时间闲置。很少有人全天运行模型，因此一台为高峰需求准备的机器，往往在许多时段仍有 GPU 和内存可用；家里有几台电脑时，这些空闲时段会分散在不同机器上。

IdleToken 想把一个人的空闲时段接到另一个人的繁忙时段：不用机器时把富余算力分享给别人，本地任务排队时再调用别人分享的模型。
