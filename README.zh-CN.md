...
| 平台 | 最低配置 |
| --- | --- |
| **Windows 10/11 · Linux x86_64/arm64** | NVIDIA 显卡，RTX 20 系（Turing，计算能力 7.5）及以后，显存 8 GB，并安装当前 NVIDIA 驱动。无需另行安装其它组件——安装包已自带所需的 CUDA 运行库。 |
...
低于该配置的电脑仍可运行客户端、控制集群，并调用他人共享的模型。
