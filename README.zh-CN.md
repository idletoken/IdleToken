...
如果一台机器装不下模型，可以再加机器。在一台机器上创建集群并生成验证码，其它机器输入验证码加入。之后的节点发现、容量探测、模型切分和权重下载都会自动完成。
...
计算节点目前支持 **Windows 和 Linux**，每台机器需要：

* NVIDIA GPU，计算能力 ≥ 7.5（RTX 20 系及以后）
* 显存 ≥ 4 GB
* Windows 驱动 ≥ 527.41
* Linux 驱动 ≥ 580.65

纯 CPU 节点会被直接拒绝，不会自动降级到 CPU 推理。
